...
export default function AuthTabs() {
  ...
  const canLogin = loginEmail.trim().includes("@") && loginPwd.trim().length >= 6;
  const canRegister = regEmail.trim().includes("@") && regPwd.trim().length >= 6 && agree;
  ...
  const doRegister = async (e) => {
    e.preventDefault();
    if (!canRegister) { setRegErr("Please fill all fields and accept terms."); return; }
    ...
    <div className="auth-wrap">
      ...
      {tab === "login" && (
        <form onSubmit={doLogin} className="row" style={{ gap: 12 }}>
          ...
          {loginErr && loginErr.split("\n").map((line, i) => (
            <div key={i} className="error">{line}</div>
          ))}
